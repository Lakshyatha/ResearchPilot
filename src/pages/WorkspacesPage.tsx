...
import { Plus, Trash2, FileText, Upload, FolderOpen, ArrowUpRight, Sparkles } from 'lucide-react';
...
export function WorkspacesPage() {
  ...
  const createWorkspace = async () => {
    ...
    if (error) {
      window.alert(`Could not create workspace: ${error.message}`);
      return;
    }
  ...
  return (
    <div>
      <div className="workspace-intro">
        <div>
          <div className="dashboard-kicker">Your research desk / overview</div>
          <h2>Make space for the next good question.</h2>
          <p>Collect the papers that shape your thinking. ResearchPilot will help you find the connections between them.</p>
        </div>
        <button
      ...
      <div className="workspace-stats"><div><span>Workspaces</span><strong>{workspaces.length.toString().padStart(2, '0')}</strong></div><div><span>Papers collected</span><strong>{workspaces.reduce((total, workspace) => total + (workspace.paper_count || 0), 0).toString().padStart(2, '0')}</strong></div><div><span>Research signal</span><strong className="signal-ready"><Sparkles size={14} /> Ready</strong></div></div>

      {workspaces.length === 0 ? (
        <div className="workspace-empty-overview">
          <FolderOpen className="w-12 h-12" />
          <h3>Your research desk is empty</h3>
          <p>Create your first workspace to start collecting papers and discovering connections.</p>
          <button className="workspace-primary-action" onClick={() => setShowCreateModal(true)}><Plus className="w-4 h-4" /> Create your first workspace</button>
        </div>
      ) : (
              ...
              <div className="space-y-4">
                {papers.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p>No papers yet. Add your first paper to start building a research signal.</p>
                    <button className="empty-state-action" onClick={() => setShowAddPaperModal(true)}>Add your first paper <ArrowUpRight size={15} /></button>
                  </div>
                ) : (
                  papers.map((paper) => (
